...
## Responsive delivery (added in the polish pass)

Cards render at roughly 165–360px, but the browser was downloading the
full-size original for every one. `src/app/media.ts` now exposes helpers that
add a `srcset` so Cloudinary resizes on the fly:
...
Candidate widths are 320/480/640/960/1280 with `f_auto,q_auto,c_fill,g_auto`.
Measured on a real asset: full-size 28,238 bytes vs `w_320` 17,611 bytes — a
**38% saving**, before the AVIF/WebP conversion `f_auto` also applies.

`imgAttrs()` works on **any** Cloudinary URL, which means admin-uploaded
gallery photos, posts and feedback images get the same treatment automatically.
Non-Cloudinary URLs (such as an `/api/media` fallback) are returned unchanged,
so nothing breaks while the manifest is still being filled in.

Applied on: home, products, menu style grids, showcase, posts and feedback.

## Admin: replacing an image

Admins can now swap the photo on an existing item **without deleting and
recreating it**. Text, links, codes and publish state are all preserved.

| Where | How |
|---|---|
| Gallery — photos you added | "Replace photo" on the card |
| Gallery — the 46 built-in showcase photos | "Replace photo" on the card |
| Posts / offers / feedback | "Replace photo" in the post actions |

Endpoints: `PUT /api/admin/gallery` with `action=replace-image` (multipart),
and `PATCH /api/admin/posts` (multipart). Both keep their original JSON
behaviour, so nothing that already worked has changed.

Two safety details worth knowing:

- Replacing a **built-in** showcase photo stores the new image as an override.
  The original Cloudinary asset is never destroyed, so **Reset restores it**.
- Replacing a **custom** upload destroys the previous asset only *after* the
  new one is saved, so a failed save can never leave you with no image.
