...
## Menu master control (new)

| File | Purpose |
|---|---|
| `src/lib/menu.ts` | Merges the built-in catalog with admin edits (name/tagline/description overrides, hidden flags) and admin-added custom products. All public pages read the menu through this. |
| `src/app/api/admin/menu/route.ts` | Admin API: GET catalog+edits, PUT category overrides, POST new custom product (with Cloudinary photo upload), PATCH/DELETE custom products. |
| `src/app/admin/MenuManager.tsx` | "Manage Menu" tab in the dashboard: edit/hide every category, add/edit/hide/remove custom products with live preview. |

Custom products automatically get their own /menu/<slug> page, appear on the home page, /products, the footer and the order form dropdown.
