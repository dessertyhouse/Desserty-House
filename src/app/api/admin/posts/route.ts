 ...
 */
/**
 * Replace the image on an existing post without recreating it.
 * Uploads the new file, points the row at it, then removes the old asset so
 * Cloudinary doesn't fill up with orphans. The post id, code, publish state
 * and text are all left exactly as they were.
 */
async function replacePostImage(request: NextRequest) {
  const formData = await request.formData();
  ...
  if (!file || !file.size) {
    return NextResponse.json({ error: 'A replacement image is required.' }, { status: 400 });
  }
  if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
  ...
  if (error) {
    // Roll back the orphan upload so a failed save leaves nothing behind.
    await cloudinary.uploader.destroy(uploadResult.public_id).catch(() => {});
    throw error;
  }

  // Only remove the previous asset once the new one is safely saved.
  if (current.cloudinary_public_id) {
    await cloudinary.uploader.destroy(current.cloudinary_public_id).catch(() => {});
  }

  await logAuditEvent({
    action: 'post_image_replaced',
    entity_type: 'post',
    entity_id: id,
    old_value: { cloudinary_public_id: current.cloudinary_public_id },
    new_value: { cloudinary_public_id: uploadResult.public_id },
    ...createAuditContext(request),
  });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Multipart PATCH = replace this post's photo, keeping the post itself.
    const contentType = request.headers.get('content-type') || '';
    if (contentType.includes('multipart/form-data')) {
      return await replacePostImage(request);
    }
    ...
    if (body.links !== undefined) updates.links = parseLinks(body.links);
