...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const clean: SiteSettings = {
      ...
      // Logo is uploaded/removed through /api/admin/logo, not typed in here.
      logoUrl: String(body.logoUrl ?? defaultSettings.logoUrl).trim().slice(0, 400),
      logoPublicId: String(body.logoPublicId ?? defaultSettings.logoPublicId).trim().slice(0, 200),
    };
