...
import { ProductIcon, IconArrowRight, IconGrid } from '@/components/Icons';
    ...
    <main>
      ...
      <section className="shell section">
        ...
        <div className="section-title-row">
          <IconGrid size={28} className="section-icon" />
          <h1>Our products</h1>
        </div>
        <p className="muted">
          Every item is baked to order in our Chennai home kitchen, with egg and eggless options on
          the bakery range and fresh hand-stretched pizzas made to order. Choose a category to see
          its style references, then order online or on WhatsApp.
        </p>
        <div className="grid">
          {visibleProducts.map((p) => (
            <article className="card" key={p.id}>
              <img
                {...imgAttrs(p.gallery[0].image)}
                alt={`${p.name} — ${p.short}`}
                loading="lazy"
                decoding="async"
                width={400}
