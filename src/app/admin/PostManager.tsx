...
export default function PostManager() {
  ...
  // Which post is currently having its photo swapped
  ...
  async function upload(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Add form data (links is an array, so it is JSON-encoded separately)
      Object.entries(formData).forEach(([key, value]) => {
        if (key === 'links') return;
        formDataToSend.set(key, String(value));
      });
      formDataToSend.set('links', JSON.stringify(formData.links || []));
      ...
      if (data.success) {
        ...
        setFormData({ title: '', description: '', kind: 'offer', links: [] });
        if (previewUrl) URL.revokeObjectURL(previewUrl);
        setPreviewUrl(null);
        form.reset();
        refresh();
  ...
  /**
   * Replace the photo on an existing post. Title, description, kind, links,
   * post code and publish state are all preserved — only the image changes.
   */
  async function replaceImage(post: Post, file: File) {
    if (!file) return;
    if (file.size > 8 * 1024 * 1024) {
      setMessage({ type: 'error', text: 'Image must be smaller than 8 MB.' });
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/admin/posts', { method: 'PATCH', body: fd });
      const data = await res.json();
      if (res.ok && data.success) {
        setMessage({ type: 'success', text: 'Photo replaced.' });
        await refresh();
      } else {
        setMessage({ type: 'error', text: data.error || 'Failed to replace the photo.' });
      }
    } catch {
      setMessage({ type: 'error', text: 'Failed to replace the photo.' });
    }
    setReplacingId(null);
  }
  ...
  const [editForm, setEditForm] = useState<PostFormData>({ title: '', description: '', kind: 'offer', links: [] });
                ...
                <div key={post.id} className={`post-card ${!post.is_published ? 'unpublished' : ''}`}>
                  ...
                  <div className="post-content">
                    ...
                    {editingPost?.id === post.id ? (
                      <form onSubmit={saveEdit} className="post-edit-form">
                        ...
                        <LinksEditor
                          compact
                          links={editForm.links}
                          onChange={(links) => setEditForm({ ...editForm, links })}
                        />
                    ...
                    <div className="post-actions">
                      ...
                      <label className="btn small outline replace-photo-btn">
                        {replacingId === post.id ? 'Replacing…' : 'Replace photo'}
                        <input
                          type="file"
                          accept="image/webp,image/jpeg,image/png"
                          disabled={replacingId === post.id}
                          onChange={(e) => {
                            const file = e.target.files?.[0];
                            e.target.value = '';
                            if (file) replaceImage(post, file);
                          }}
                        />
                      </label>
                      <button
