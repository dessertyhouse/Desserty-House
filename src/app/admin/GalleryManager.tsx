...
export default function GalleryManager() {
  ...
  /**
   * Swap the photo on any gallery item — built-in or admin-added — without
   * touching its text, links or hidden state. Built-in originals are kept in
   * Cloudinary, so "Reset" always restores the stock photo.
   */
  async function replacePhoto(code: string, file: File) {
    if (!file) return;
    if (file.size > 8 * 1024 * 1024) {
      notify('error', 'Image must be under 8 MB.');
      return;
    }
    setBusy(code);
    try {
      ...
      const res = await fetch('/api/admin/gallery', { method: 'PUT', body: fd });
      const data = await res.json();
      if (data.success) {
        setOverrides(data.gallery?.overrides || {});
        setCustom(data.gallery?.custom || []);
        notify('success', 'Photo replaced.');
      } else {
        notify('error', data.error || 'Failed to replace the photo.');
      }
    } catch {
      notify('error', 'Failed to replace the photo.');
    }
    setBusy(null);
  }
          ...
          {custom.map((c) => (
            <div className={`style-card ${c.hidden ? 'is-hidden' : ''}`} key={c.code}>
              ...
              <label className="replace-photo">
                <IconUpload size={13} />
                <span>{busy === c.code ? 'Replacing…' : 'Replace photo'}</span>
                <input
                  type="file"
                  accept="image/webp,image/jpeg,image/png"
                  ...
                  onChange={(e) => {
                    ...
                    e.target.value = '';
                    if (file) replacePhoto(c.code, file);
                  }}
                />
              </label>
              <input
            ...
            {filteredBuiltIn.map((item) => {
              ...
              return (
                <div className={`style-card ${eff.hidden ? 'is-hidden' : ''}`} key={item.code}>
                  <img
                    src={overrides[item.code]?.image_url || item.image}
                    alt={eff.title}
                    loading="lazy"
                  />
                  <span className="style-code">
                    ...
                    {overrides[item.code]?.image_url ? ' · NEW PHOTO' : ''}
                  </span>
                  <label className="replace-photo">
                    <IconUpload size={13} />
                    <span>{busy === item.code ? 'Replacing…' : 'Replace photo'}</span>
                    <input
                      type="file"
                      accept="image/webp,image/jpeg,image/png"
                      ...
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        e.target.value = '';
                        if (file) replacePhoto(item.code, file);
                      }}
                    />
                  </label>
                  <input
