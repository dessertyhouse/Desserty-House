...
type Override = { title?: string; category?: string; description?: string; hidden?: boolean };
type CustomItem = {
  code: string;
  title: string;
  category: string;
...
export default function GalleryManager() {
  ...
  function effective(item: BuiltIn) {
    ...
    return {
      ...
      hidden: !!o.hidden,
      edited: !!(o.title || o.category || o.description),
    };
  }
  ...
  async function addPhoto(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/admin/gallery', { method: 'POST', body: new FormData(form) });
      const data = await res.json();
      if (data.success) {
        setCustom(data.gallery.custom || []);
        form.reset();
